...
#  openssl ca -in ${1}req.pem -out ${1}cert.pem
  openssl x509 -req -in ${1}req.pem -sha1 -CA root.pem -CAkey root.pem -CAcreateserial -out ${1}cert.pem -days 365
#  openssl x509 -req -in ${1}req.pem -sha1 -extfile /etc/ssl/openssl.cnf -extensions usr_cert -CA root.pem -CAkey root.pem -CAcreateserial -out ${1}cert.pem -days 365
